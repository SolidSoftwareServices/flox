...
{
	internal class SapRepositoryOfCrmUmcOptions : SapRepositoryOptions
	{
		public SapRepositoryOfCrmUmcOptions(ISapSettings sapSettings, ISapResultStatusHandler apiResultHandler,
			...
			: base($"{sapSettings.SapBaseUrl}{sapSettings.CrmUtilitiesUmcEndPoint}", apiResultHandler,
				userSessionProvider,
				sapSessionData, oDataSettings, profiler, sapSettings.BatchEnlistTimeoutMilliseconds,
				sapSettings.RequestTimeout, modelValidator, edmxResolver,tokenProvider)
		{
		...
		public override async Task<string> TokenProviderUrlAsync() =>
			await _sapSettings.SapCrmUmcBearerTokenProviderUrlAsync();
