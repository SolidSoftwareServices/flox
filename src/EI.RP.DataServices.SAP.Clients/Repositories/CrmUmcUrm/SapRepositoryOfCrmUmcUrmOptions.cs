...
{
	internal class SapRepositoryOfCrmUmcUrmOptions : SapRepositoryOptions
	{
		public SapRepositoryOfCrmUmcUrmOptions(ISapSettings sapSettings, ISapResultStatusHandler apiResultHandler,
			...
			: base($"{sapSettings.SapBaseUrl}{sapSettings.CrmUtilitiesUmcUrmEndPoint}", apiResultHandler,
				userSessionProvider, sapSessionData, oDataSettings, profiler, sapSettings.BatchEnlistTimeoutMilliseconds, sapSettings.RequestTimeout, modelValidator, edmxResolver,tokenProvider)
		{
		...
		public override async Task<string> TokenProviderUrlAsync() =>
			await _sapSettings.SapCrmUmcUrmBearerTokenProviderUrlAsync();
	}
}