...
{
    [TestFixture]
    class WhenInMovingHomeStepClosePage_OnlyGas : MyAccountCommonTests<StepCloseAccountPage>
    {
		...
		public async Task CanSeeComponents()
		{
			...
			var movingOutSelectedDateTime = gasUserInfo.ContractStartDate == DateTime.Now.AddDays(-1)
				? DateTime.Now.Date
	    ...
	    private CloseAccountsCommand BuildCommand(AddressInfo addressInfo)
	    {
		    var cmd = new CloseAccountsCommand(ClientAccountType.Gas, addressInfo,
			    ...
			    meterReadingGasAccount: GasConfigurator.ToGasMeterReading(Sut), moveOutIncommingOccupantInfo: new MoveOutIncommingOccupantInfo());
		    return cmd;
	    }
