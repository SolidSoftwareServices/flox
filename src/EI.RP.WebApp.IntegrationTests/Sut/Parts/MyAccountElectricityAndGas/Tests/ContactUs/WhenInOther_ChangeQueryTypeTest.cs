...
{
    [TestFixture]
    internal class WhenInOther_ChangeQueryTypeTest : WhenInContactUsPageTest
    {
        protected override async Task TestScenarioArrangement()
        {
            await base.TestScenarioArrangement();

            Sut = (await Sut.ChangeQueryType(ContactQueryType.Other)).CurrentPageAs<ContactUsPage>();
        }
        ...
        public async Task CanSubmitQuery()
        {
            ...
            var cmd = new UserContactRequest(UserConfig.Accounts.Single().Partner, Sut.SelectedAccount.Value,
                null, Sut.SubjectInput.Value, Sut.QueryInput.Value, ContactQueryType.Other);
