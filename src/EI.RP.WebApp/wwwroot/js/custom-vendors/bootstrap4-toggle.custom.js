...
+function ($) {
	...
	Toggle.prototype.render = function () {
		...
		var $toggleOn = $('<label for="'+ this.$element.prop('id') +'" class="btn" aria-hidden="true">').html(this.options.on)
			.addClass(this._onstyle + ' ' + size);
		var $toggleOff = $('<label for="'+ this.$element.prop('id') +'" class="btn" aria-hidden="true">').html(this.options.off)
			.addClass(this._offstyle + ' ' + size);
		var $toggleHandle = $('<span class="toggle-handle btn btn-light">')
			.addClass(size);
		var $toggleGroup = $('<div class="toggle-group">')
			.append($toggleOn, $toggleOff, $toggleHandle);
		var $toggle = $('<div class="toggle btn" data-toggle="toggle" role="checkbox" tabindex="0">')
			.attr('aria-checked', this.$element.prop('checked'))
			.addClass( this.$element.prop('checked') ? this._onstyle : this._offstyle+' off' )
			.addClass(size).addClass(this.options.style);

		if (this.$element.attr('aria-labelledby')) {
			$toggle.attr('aria-labelledby', this.$element.attr('aria-labelledby'));
		}

		if (this.$element.attr('aria-label')) {
			$toggle.attr('aria-label', this.$element.attr('aria-label'));
		}
	...
	Toggle.prototype.on = function (silent) {
		...
		this.$toggle.attr('aria-checked', true);
	...
	Toggle.prototype.off = function (silent) {
		if (this.$element.prop('disabled')) return false;
		this.$toggle.attr('aria-checked', false);
	...
	$(document).on('keypress.bs.toggle', 'div[data-toggle^=toggle]', function(e) {
		if(e.type === 'keypress'){
