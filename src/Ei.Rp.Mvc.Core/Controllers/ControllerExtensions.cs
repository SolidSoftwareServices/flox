...
namespace Ei.Rp.Mvc.Core.Controllers
{
	...
	public static class ControllerExtensions
    {
		...
		public static async Task<IActionResult> HandleActionAsync(this Controller targetController,
			...
			Action<DomainException> onCustomErrorHandling = null)
		{
		...
		private class HandleActionRequest
		{
			...
			public HandleActionRequest(Controller targetController, Func<Task<IActionResult>> controllerActionHandler, Func<IErrorsViewModel, Task<IActionResult>> onErrorUseThisResultProvider, IErrorsViewModel storeErrorsOn, Action<DomainException> onCustomErrorHandling)
			{
			...
			public Action<DomainException> OnCustomErrorHandling { get; }
		}
	        ...
	        AggregateException domainExceptions)
        {
            ...
            if (domainExceptions.InnerExceptions.Any(x => !(x is DomainException) && !(x is ValidationException)))
            {
                ...
                throw new AggregateException(domainExceptions.InnerExceptions.Where(x =>
                    !(x is DomainException) && !(x is ValidationException)));
            }

            foreach (var ex in domainExceptions.InnerExceptions.Where(x => x is DomainException)
                .Cast<DomainException>())
            {
                ...
                else
                {
                    ...
                    if (ex.DomainError.ErrorMessage.Count(x => x == '#') == 2)
                    {
                        Logger.Warn(() => "The default message for the domain error contains replacement tokens that have not been resolved.");
                    }

                    //key= "" --> adds to the validation summary
                    request.TargetController.ModelState.AddModelError(string.Empty, ex.DomainError.ErrorMessage);
                }
            ...
            void LogError()
            {
	            ...
	            foreach (var exception in domainExceptions.InnerExceptions)
	            {
		            if (exception is DomainException)
		            {
			            ...
			            sb.AppendLine($"{exception} - extra:{ex.ExtraInfo}");
		            }
		            else
		            {
