...
namespace S3.CoreServices.System
{
	public class TypesFinder
	{
		...
		public Type FindType(string typeName,bool includePublicTypesOnly=true,bool caseSensitive=true)
		{
			var type = FindTypes(x => caseSensitive && x.FullName == typeName 
			                          ||!caseSensitive && x.FullName?.Equals(typeName,StringComparison.InvariantCultureIgnoreCase)==true, typeName,includePublicTypesOnly)
				.SingleOrDefault();
			if (type == null)
			{
				try
		...
		{
			var avoid = new[] {"Microsoft.TestPlatform.Utilities", "System.Threading.AccessControl", "TechTalk.SpecRun.Common", "S3.WebApp.Views" };
			...
			do
			{
				...
				{
					if (reference.FullName.StartsWith("S3",StringComparison.InvariantCultureIgnoreCase)
