...
namespace S3.Mvc.Core.System.Request
{
	...
	public static class QueryCollectionExtensions
	{
		...
		public static ExpandoObject ToExpandoObject(this RouteValueDictionary routeValues, IQueryCollection requestQuery,bool forceLowerCaseOfValues=true)
		{
			...
			var src= routeValues.Where(x => x.Key != null && !x.Key.StartsWith(Prefixes.ContainerProperty) && !Exclusions.Contains(x.Key.ToLower()))
				.Select(x=>
				{
					...
					if (forceLowerCaseOfValues)
					{
						v = v?.ToLowerInvariant();
					}
					return new KeyValuePair<string, object>(x.Key?.ToLowerInvariant(), v);
				});
